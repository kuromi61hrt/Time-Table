...
def build_dataset(form: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, row in enumerate(rows, 1):
        ...
        if kind == "extra":
            fixed.extend(_reserve_fixed(subject_id, staff_id, theory_hours, "PLACEMENT", occupied))
...
def _reserve_fixed(subject_id: str, staff_id: str, hours: int, kind: str,
                   occupied: set[tuple[str, int]]) -> list[dict[str, Any]]:
    if not hours:
        return []
    ...
    while remaining:
        size = min(remaining, 2) if kind == "LAB" else 1
        choice = next(((day, block[:size]) for day, block in candidates
                       if all((day, period) not in occupied for period in block[:size])), None)
        if choice is None:
