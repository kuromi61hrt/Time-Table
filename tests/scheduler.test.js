...
import { createDefaultSlots, generateTimetable, checkSchedule } from '../src/scheduler.js';
...
test('permits exactly one R12 theory repeat on an externally fixed lab day', () => {
  ...
  assert.equal(result.diagnostics.some(d => d.rule === 'R12' && d.severity === 'warning'), true);
});
...
test('reserves different first-period subjects across the six days', () => {
  ...
  assert.deepEqual(firstPeriodSubjects, ['s0', 's1', 's2', 's3', 's4', 's5']);
});
...
test('keeps auto lab blocks away from the six distinct theory first-hour slots', () => {
  ...
  const result = generateTimetable(model);
  assert.deepEqual(model.settings.days.map(day => result.schedules.AIDS[day][0]?.subjectId), ['s0', 's1', 's2', 's3', 's4', 's5']);
...
test('keeps an input-declared lab day when the start period is not supplied', () => {
  ...
  assert.deepEqual(result.schedules.AIDS.TUE.filter(cell => cell?.kind === 'lab').map(cell => cell.period), [0, 1]);
});
